...
type RequestType = { workspaceId: Id<"workspaces"> };       // Tipado de los argumentos de la mutation de convex
...
export const useNewJoinCode = () => { // Hook para actualizar el codigo de unión a un workspace
  ...
  const mutation = useMutation(api.workspaces.newJoinCode) // Definición de la mutation de la api de convex para actualizar el codigo de unión a un workspace
  ...
  const mutate = useCallback(async(values:RequestType, options?: Options) => { // Ejecución de la mutation -> callbacks
    try {
      ...
      const response = await mutation(values);       // Cuando se usa la mutation en el modal, se pasa el workspaceId
