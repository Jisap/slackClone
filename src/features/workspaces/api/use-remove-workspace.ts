...
type RequestType = { id: Id<"workspaces"> };                // Tipado de los argumentos de la mutation de convex
...
export const useRemoveWorkspace = () => { // Hook para borrar un workspace
  ...
  const mutation = useMutation(api.workspaces.remove) // Definición de la mutation de la api de convex para borrar un workspace
  ...
  const mutate = useCallback(async(values:RequestType, options?: Options) => { // Ejecución de la mutation -> callbacks
    try {
      ...
      const response = await mutation(values);       // Cuando se usa la mutation en el modal, se pasa el id del workspace
